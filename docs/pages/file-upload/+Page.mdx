...
(1): Only one file chunk at a time is loaded in memory. Thus, memory consumption is low and constant (the chunk size). Recommended if files are expected to be large.  
(2): The whole file is loaded in memory. For large files (e.g. a large video) this leads to prohibitively high memory usage.
</div>
...
This zero-buffering design comes with the following inherent limitations.

### One-shot reads

Each file can only be read once — calling `.stream()`, `.text()`, or `.arrayBuffer()` a second time throws an error.

> For efficiency, the HTTP stream isn't buffered and can therefore only be consumed once.
