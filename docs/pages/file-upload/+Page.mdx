...
> When a telefunction call contains files, Telefunc automatically switches from JSON to a binary format to stream the file bytes efficiently.
...
Alternatively, with Express, Fastify, or any Node.js framework, you can pass the request as a [`Readable`](https://nodejs.org/api/stream.html#readable-streams) stream along with the headers:
...
Telefunc uses a custom binary stream parser — files are **not** buffered into memory on the server.

1. The client serializes the telefunction call into a binary request: call metadata first, followed by raw file bytes.
