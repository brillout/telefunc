...
Pass the web standard [`Request`](https://developer.mozilla.org/en-US/docs/Web/API/Request) object directly:

```ts
const httpResponse = await telefunc({ request })
```

Or with Express, Fastify, or any Node.js framework, pass the [`Readable`](https://nodejs.org/api/stream.html#readable-streams) stream:
