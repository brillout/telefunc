...
In principle,
if our frontend is bug-free and our telefunctions are only called by our frontend,
then `throw Abort()` is not called.

There are situations when, it is expected that the frontend makes calls without the right permissions; we then `return` instead of `throw Abort()`.
