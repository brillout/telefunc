...
function Comment({ id, text, context }) {
  const deleteButton =
    context.user.isAdmin ?
      ...
      null // We only show the delete button to admins
  return <>
    <p>{ text }</p>
    { deleteButton }
  </>
...
If is expected that our frontend can sometimes make calls without the right permissions, we `return` instead of `throw Abort()`.
