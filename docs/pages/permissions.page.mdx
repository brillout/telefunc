...
We use `Abort()` even if our frontend always calls telefunctions with the right permissions:
...
For more convenience we can use `shield()` instead of `throw Abort()` to ensure the arguments' type.
...
shield(onCommentDelete, [shield.type.number])
export async function onCommentDelete(id) {
  // `shield()` ensures that `id` is a number
