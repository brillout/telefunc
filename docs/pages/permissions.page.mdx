...
>
> We can use implement `getContext()` wrappers and permission functions for advanced permission rules and increased safety.
...
> We don't use `throw Abort()` but `throw new Error()` in ``throw new Error('Wrong findTodoItem() usage')`` because it is <Link text="not a public but an internal error" href='/abort-or-error' />.


## Permission functions

We can implement a permission function that is guaranteed to be applied to all our telefunctions:
