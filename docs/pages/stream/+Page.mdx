...
| <Link text={<code>channel().sendBinary()</code>} href="/channel#binary-data" /> | No | Fire-and-forget binary — low-latency frames where dropping is acceptable |
| <Link text={<code>pubsub().publishBinary()</code>} href="/channel#pubsub" /> | Publish-side only | Broadcast binary — video to multiple subscribers |
