...
Telefunctions can stream values to the client using `AsyncGenerator`, `ReadableStream`, or nested `Promise`s. A single telefunction can return any combination of these — all multiplexed over one HTTP response.
...
|---|---|---|
| `AsyncGenerator` | AI tokens, notifications, progress updates | Structured (JSON-serializable) |
| `ReadableStream` | File downloads, media, raw bytes | Binary (`Uint8Array`) |
| Nested `Promise` | Lazy parts of a response that resolve independently | Any serializable value |
...
A telefunction can return an object with any combination of generators, streams, and promises — all multiplexed over one HTTP response.
...
> All streamed values are multiplexed over a single HTTP response. Multiple generators and promises resolve concurrently without blocking each other.
