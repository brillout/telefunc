...
You may be tempted to create generic telefunctions but it's recommended against it. Instead, implement what is called *event-based telefunctions*.
...
In the example below, the reasons why event-based telefunctions lead to increased:
 - Development speed (while keeping telefunctions DRY)
 - Security
 - Performance
...
It's recommended to name telefunctions `onSomeEvent()` (see <Link href="#naming-convention"/>), because telefunction calls are always triggered by some kind of event — typically a user action, such as the user clicking on a button.

```shell
# Also: co-locate .telefunc.js files
...
That's why event-based telefunctions are recommended, along with the naming convention to ensure telefunctions are tightly coupled to components.
...
As explained in <Link href="#example">the example above</Link>, for a clear structure and proficient Telefunc usage, the following convention is recommended.
