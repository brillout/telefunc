...
The first-party Redis adapter wires pub/sub to Redis with a single config line:
...
import { RedisTransport } from '@telefunc/redis'
import Redis from 'ioredis'

config.pubsub.transport = new RedisTransport({
  publisher: new Redis(),
  subscriber: new Redis(),  // Redis requires a dedicated connection for subscriptions
})
```

Telefunc assigns a global `seq` per key via Redis `INCR`, so subscribers see messages in order even across nodes.
