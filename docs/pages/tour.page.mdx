...
> - [Permissions & validation](#permissions-validation)
...
Our `run()` telefunction exposes our entire database to the world;
anyone can make such HTTP request:
...
So, telefunctions are public and need protection.
...
Not only does `shield()` call `new Abort()` on our behalf, but it also infers the arguments type for IntelliSense and TypeScript.
