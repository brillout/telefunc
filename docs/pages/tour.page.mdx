...
Telefunctions such as `hello()` are *always* called on the server-side,
which means that telefunctions are able to run SQL/ORM queries.
...
In essence, Telefunc enables our frontend to use any SQL/ORM queries.
...
> Our `onNewTodo` telefunction is missing a `shield(onNewTodo, [t.string])` protection, which we'll talk about in a moment.
...
This means our frontend can fully tap into the power of our SQL/ORM engine. This is both simpler and more powerful than REST/GraphQL.

> We need GraphQL or REST only if a) we have third parties that access our database, or b) we need to decouple frontend-backend deployment.
> Otherwise RPC is both simpler and more powerful.
...
Thus, this telefunction is not safe:
...
As anyone can this HTTP request:
...
async function run(sql) {
  // We talk about `getContext()` in a moment
...
it is the frontend development/developer that owns the telefunctions.

This is the opposite of GraphQL/REST where it is the backend development/developer that owns the GraphQL/RESTful API.

This inversion of control leads to a fundamentally clearer seperation of concerns.
...
Our frontend seemlessly uses our backend telefunctions's type:
