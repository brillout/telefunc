...
> but note that Telefunc works with any UI framework (React, Vue, Angular, Svelte, ...).
...
> A GraphQL/RESTful API is needed if we want to enable third parties to access our database,
...
we cannot assuming anything about the arguments type.
...
But, if we want to pass information to the frontend,
...
This enables the frontend, for example, to redirect the user to the login page if the user is not logged in.
...
export async function onFormSubmit(email, password) {
  if (!isStrongPassword(password)) {
    return { wrongPassword: "Password is too weak." }
  }
  /* ... */
}
```

> We explain the benefits of Telefunc's approach at <Link href='/permissions' />

The server:

```
Error: The telefunction `TodoItem.telefunc.js:onTextChange` returned `throw Abort()`. (This error is not shown in production, see https://telefunc.com/permissions )
    at hello (TodoItem.telefunc.js:18)
```

The browser:

```
Uncaught Error: The telefunction `/hello.telefunc.js:hello` returned `throw Abort()`.
    at makeHttpRequest (telefunc/client.js)
    at index.html
```
