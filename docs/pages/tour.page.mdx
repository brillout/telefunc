...
Instead of defining types twice, we can let `shield()` infer the arguments' type:

```ts
// UserSettings.telefunc.ts
// Environment: Node.js
...
const onUserSettingsChange = shield(
  [
    t.or(
      { settingName: t.const('firstName'), newValue: t.string },
      { settingName: t.const('age'), newValue: t.number }
    )
  ],
  // TypeScript infers `settingName` and `newValue` to
  //   { settingName: 'firstName' as const, newValue: string } |
  //   { settingName: 'age' as const, newValue: number }
  async ({ settingName, newValue }) => {
    if (settingName === 'firstName') {
      // TypeScript knows that `newValue` is a string in this if-block
      newValue = newValue + 'Some Last Name' // ✅
      newValue = newValue + 1 // ❌ TypeScript complains that `newValue` is not a number
    }
    if (settingName === 'age') {
      // TypeScript knows that `newValue` is a number in this if-block
      newValue = newValue + 1 // ✅
    }
  }
)
...
With Telefunc, the frontend seamlessly uses backend types:
...
// works out-of-the-box: type safety, IntelliSense and autocomplete just works.
    ...
    <form>
      <input
        ...
        onChange={ev =>
          ...
          onUserSettingChange({ settingName: 'firstName', newValue: ev.target.value })
        }
      >
      ...
      <input
        ...
        onChange={ev =>
          ...
          onUserSettingChange({ settingName: 'age', newValue: ev.target.value })
        }
      >
        {initialState.age}
      </input>
