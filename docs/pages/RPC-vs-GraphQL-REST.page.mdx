...
> We recommand taking the <Link href="/tour" text="Telefunc Tour" /> before reading this document.
> It explains what RPC is, and shows how a Telefunc app is typically structured.
...
and also enable us to develop multiple frontends with RPC.
</Note>
...
But we can decouple the frontend development from the backend development by using a Telefunc Server: a dedicated Node.js server with sole purpose of serving telefunctions.
...
> Most of the time, we don't need a Telefunc Server and we can use Telefunc simply [as a server middleware](https://telefunc.com/tour#how-it-works) instead.
...
Also, using a GraphQL API can be crucial strategy for scaling large teams.
