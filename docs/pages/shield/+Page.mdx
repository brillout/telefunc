...
Automatic TypeScript `shield()` generation remains the default and most transparent option. Manual `shield()` is mainly useful for JavaScript users, for stacks that don't transpile server-side code, or when bringing your own runtime validator.

Automatic `shield()` generation is based on your telefunction's TypeScript signature. It doesn't inspect Zod schemas or other runtime validators: if you bring your own validator, keep the telefunction's TypeScript type in sync with it.
