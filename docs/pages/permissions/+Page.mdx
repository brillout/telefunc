...
In general, use `throw Abort()` upon permission denials but, sometimes, the frontend needs to know why the the telefunction call failed:
in this example returning `{ notLoggedIn: true }` instead of `throw Abort()` so that the frontend can perform a redirection:
