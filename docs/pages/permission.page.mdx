...
Telefunctions are public: anyone in the world can call your telefunctions.
...
export async function getTodoList() {
  ...
  if( isLoggedIn ) {
    // Instead of `throw new Abort()` we return aplain JavaScript object
    // `{ isNotLoggedIn: true }`.
    // The frontend can then, for example, redirect the user to the login page.
