...
> This page is about handling bugs, expeced errors, and network errors.
>
...
function onFromSubmit(data) {
  try {
    ...
    return {
      errorMessage: 'Data is invalid. Please enter valid data.'
    }
  }
}
```
...
function onFromSubmit(data) {
  try {
    ...
    // In general, we recommend using `throw
    throw Abort({ validationError: 'Data is invalid' })
  }
}
```
...
> We need to catch the error and use a `throw Abort(someValue)` instead because Telefunc doesn't send the original `Error` object to the frontend (in order to avoid leaking sensitive information).
