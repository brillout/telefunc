...
If a generator throws a regular error (not <Link text={<code>Abort()</code>} href="/Abort" />) mid-stream, the client receives an error with message `'Internal Server Error'`. Values yielded before the error are preserved.
...
> To avoid leaking sensitive information, Telefunc replaces the original error with a generic `'Internal Server Error'` message — same as with non-streaming telefunctions.
