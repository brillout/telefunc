...
// This doesn't actually load CreateTodo.telefunc.js — explained in the next section
...
> Name and co-locate `TodoList.telefunc.js` next to `TodoList.jsx` — a practice explained at <Link href="/event-based" />.
...
For example, anyone can call the `hello()` telefunction seen in the previous section by opening a Linux terminal and making this HTTP request:
...
In the sections below <Link href="#throw-abort"/> and <Link href="#shield" /> it's explained how to achieve that.
...
> In principle, you could also `throw new Error()` instead of `throw Abort()` as it also interrupts the telefunction call. But it's recommended to use `throw Abort()` as it comes with many conveniences.
...
For efficient telefunctions, it's recommended to create tailored telefunctions instead of generic ones — see <Link href="/event-based" />.
