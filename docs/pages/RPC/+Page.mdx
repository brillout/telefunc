...
> We will show later <Link href="#how-it-works">how it works</Link>.

> This practice &mdash; calling functions remotely in a seamless fashion &mdash; is known as [_**R**emote **P**rocedure **C**all (RPC)_](https://en.wikipedia.org/wiki/Remote_procedure_call).
...
>
> If you save secret information in another file then we recommend using your framework to guarentee that the secret information is never accidentally leaked to the client-side (and thus made public).
> For example you can add `.server.js` to the filename [when using Vike](https://vike.dev/file-env) as well as [when using SvelteKit](https://kit.svelte.dev/docs/server-only-modules).
...
> We collocate and name the `TodoList.telefunc.js` file after `TodoList.jsx`, which is a practice we recommend and explain at <Link href="/event-based" />.
