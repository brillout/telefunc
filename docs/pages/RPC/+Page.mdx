...
> You can `throw new Error()` to interrupt the telefunction call, but Telefunc treats these as unhandled exceptions. Use `throw Abort` to handle known errors gracefully. See <Link href="/abort-vs-error" />.
