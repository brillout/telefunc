...
The context object returned by `getContext()` also includes `onConnectionClose()` — a callback that fires when the request lifecycle ends for any reason (response sent, stream complete, or client disconnect). Fires exactly once. Use it to release resources.
