...
The context object returned by `getContext()` also includes `onConnectionAbort()` — a callback that fires when the client disconnects before the response completes. Use it to cancel upstream work or release resources.
