...
The context object returned by `getContext()` also includes `onConnectionAbort()` — a callback that fires when the client disconnects. Does NOT fire on normal response completion.

```ts
const context = getContext()
context.onConnectionAbort(() => {
  // Client disconnected — cancel upstream work
})
```

## `onConnectionClose()`

`onConnectionClose()` fires when the request lifecycle ends for any reason (response sent, stream complete, or client disconnect). Fires exactly once. Use it to release resources.
...
See <Link href="/cancellation" /> for full documentation.
