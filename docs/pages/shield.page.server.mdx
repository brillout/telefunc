...
If we use TypeScript, then Telefunc automatically generates a `shield()` for each telefunction.

In other words, Telefunc automatically checks the argument types at runtime:
...
> Telefunc's automatic `shield()` generation only works for stacks that transpile server-side code (Next.js, Vite, vite-plugin-ssr, SvelteKit, Nuxt, etc.).
>
> For stacks that don't transpile server-side code (e.g. React Native, CRA, Parcel), we need to define `shield()` manually ourselves.

If we define `shield()` manually (instead of using Telefunc's automatic `shield()` generator), then note we can use `shield()` to infer types:
