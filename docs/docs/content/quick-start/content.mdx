...
```ts title="components/TodoList.telefunc.ts" env=server
...
```jsx choice=react title="components/TodoList.jsx" env=client
...
```html choice=vue title="components/TodoList.vue" env=client
<script>
...
```jsx choice=solid title="components/TodoList.jsx" env=client
import { onNewTodo } from './TodoList.telefunc'
...
```html choice=svelte title="src/routes/TodoList.svelte" env=client
...
```js title="components/TodoList.telefunc.js" env=server
