...
function generateShieldCode({
  ...
  type Discovered = { e: ExportList[number]; mainShield: string | undefined; returnSites: Site[]; argumentSites: Site[] }
  ...
  for (const e of exportList) {
    ...
    walkSites(argsAlias.getType(), `${e.exportName}Args`, argsAlias, 'return', (t) => `\`[\${ShieldStr<Awaited<ReturnType<${t}>>>}]\``, argumentSites)
  ...
  const processSide = (d: Discovered, sites: Site[], aliasSuffix: string, applyFn: string) => {
    ...
    assert(values.length === sites.length, `${d.e.exportName}${aliasSuffix}: ${values.length} parts, expected ${sites.length}`)
...
function walkSites(
  ...
  if (shieldsProp) {
    for (const entry of shieldsProp.getTypeAtLocation(locationNode).getProperties()) {
      // `never` means this entry carries no data (e.g. `ChannelData<never>`) — skip so we don't
      // emit an unresolvable ShieldStr<never> expression that would poison the combined literal.
      if (entry.getTypeAtLocation(locationNode).isNever()) continue
...
function formatEntries(sites: readonly { segments: string[]; name: string }[], values: readonly string[]): string | null {
  if (sites.length === 0) return null
