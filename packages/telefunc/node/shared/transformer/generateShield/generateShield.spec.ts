...
describe('generateShield', () => {
  it('generateShield, one telefunction', async () => {
    ...
    expect(shieldedSrc).toMatchInlineSnapshot(`
      ...
      __telefunc_shield([__telefunc_t.string], doSomething, { __autoGenerated: true });
      "
    `)
  })
...
export function doSomethingElse(arg: string | number, arg2: { val?: number }) {
    ...
    expect(shieldedSrc).toMatchInlineSnapshot(`
      ...
      const __telefunc_t = __telefunc_shield.type;
      __telefunc_shield([__telefunc_t.string], doSomething, { __autoGenerated: true });
      __telefunc_shield([__telefunc_t.or(__telefunc_t.number, __telefunc_t.string), { val: __telefunc_t.optional(__telefunc_t.number) }], doSomethingElse, { __autoGenerated: true });
      "
    `)
  })
...
export enum Attending {
    ...
    expect(shieldedSrc).toMatchInlineSnapshot(`
      "import { shield as __telefunc_shield } from "telefunc";
      const __telefunc_t = __telefunc_shield.type;
      __telefunc_shield([__telefunc_t.string, __telefunc_t.or(__telefunc_t.const('UNKNOWN'), __telefunc_t.const('NOT_ATTENDING'), __telefunc_t.const('ATTENDING'), __telefunc_t.const('MAYBE'))], onFoo, { __autoGenerated: true });
      "
    `)
  })
...
const someFunc = async ({ name }: { name: string }) => {
  ...
  return { message }
    ...
    expect(codeShield).toMatchInlineSnapshot(`
      "import { shield as __telefunc_shield } from "telefunc";
      const __telefunc_t = __telefunc_shield.type;
      __telefunc_shield([{ name: __telefunc_t.string }], someFunc, { __autoGenerated: true });
      "
    `)
  })
})
