...
function installShield(telefunction: Function, telefunctionShield: any, generated: boolean) {
  ...
  ;(telefunction as any as Record<any, unknown>)[shieldMetaKey as any] = { autoGenerated: generated }
}
...
function shieldIsAutoGenerated(telefunction: Telefunction): boolean {
  const meta = (telefunction as any)[shieldMetaKey]
...
function detectExtraProperties(verifier: unknown, arg: unknown, breadcrumbs: string = '[root]'): string[] {
  const extraProps: string[] = []

  if (isPlainObject(verifier) && isPlainObject(arg)) {
    const shieldKeys = Object.keys(verifier)
    const argKeys = Object.keys(arg)

    for (const key of argKeys) {
      if (!shieldKeys.includes(key)) {
        extraProps.push(`${breadcrumbs} > [object: key \`${key}\`]`)
      } else {
  ...
  } else if (Array.isArray(verifier) && Array.isArray(arg)) {
    // Handle tuple validation - check if there are extra elements
    if (arg.length > verifier.length) {
      for (let i = verifier.length; i < arg.length; i++) {
        extraProps.push(`${breadcrumbs} > [tuple: element ${i}]`)
      }
    }
...
function verifyObject(obj: Record<string, unknown>, arg: unknown, breadcrumbs: string): true | string {
  ...
  // Only validate keys that are defined in the shield definition (obj)
  // This makes the validation more permissive like TypeScript - extra properties are allowed
