...
>(verifiers: ArgumentVerifyers, telefunction: Telefunction, options?: ShieldOptions): Telefunction {
  assertUsage(isTelefunction(telefunction), 'shield(verifiers, telefunction)')
...
function assertShield(telefunctionShield: unknown): asserts telefunctionShield is TelefunctionShield {
  assertUsage(
    ...
    [
      '[shield(verifiers, telefunction)]',
      '`verifiers` should be an array.',
      `Example of correct usage: \`shield([shield.type.string], telefunction)\`.`,
      `Example of wrong usage: \`shield(shield.type.string, telefunction)\`.`,
    ].join(' '),
  )
}
