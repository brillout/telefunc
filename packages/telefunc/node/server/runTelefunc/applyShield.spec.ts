...
describe('applyShield', () => {
  ...
  it('should warn about superfluous properties in auto-generated shields when enabled', () => {
    ...
    try {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Shield Superfluous Properties Warning')
      )
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Extra properties: [root] > [tuple: element 0] > [object: key `city`], [root] > [tuple: element 0] > [object: key `country`]')
      )
    } finally {
  ...
  it('should still reject invalid requests', () => {
    ...
    try {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining('Shield Validation Error')
      )
    } finally {
      consoleSpy.mockRestore()
    }
