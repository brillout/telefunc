...
function getContext_async(): Telefunc.Context {
  ...
  // if no user context was provided, so augmentContext() can attach onConnectionAbort()/onConnectionClose().
...
function restoreContext_async(context: null | Telefunc.Context): any {
  ...
  // a telefunc execution even without provideTelefuncContext() — needed for onConnectionAbort()/onConnectionClose().
  globalObject.asyncStore = globalObject.asyncStore ?? new AsyncLocalStorage()
