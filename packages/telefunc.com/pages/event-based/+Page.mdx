...
>
> This page explains how to use Telefunc in a way that significantly increases development speed. We recommend reading it thoroughly — it's worth it!
>
...
In the example below, we explain why event-based telefunctions lead to:
 - Increased development speed. (And we explain how to keep things DRY.)
 - Better security.
 - Improved speed.
...
> With Telefunc, you think in terms of what the frontend needs, instead of thinking of the backend exposing data. From that perspective, it's a lot more natural to co-locate telefunctions next to UI components.
...
1. It isn't safe.
   > As explained at <Link href="/RPC" />, telefunctions are public. This means the user can call `updateTask({ author: Math.floor(Math.random()*100000) })` which is a big security issue.
...
- Decrease <Link href="/RPC#speed">RPC speed</Link>.

Instead, we recommend implementing a tailored telefunction:
...
When a telefunction is tightly coupled with a component:
 - The telefunction's return value can be minimal (exactly and only what is needed by the component) leading to improved speed.
 - The telefunction's arguments can be minimal (exactly and only what is needed by the component) leading to better security.
